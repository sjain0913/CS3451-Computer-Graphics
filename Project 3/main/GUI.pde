...
void keyPressed()  // executed each time a key is pressed: sets the Boolean "keyPressed" till it is released   
    ...
    if(key=='o') 
      {
      Sites.resetPointsOnCircle(27);
      }
    //if(key=='p') ; // used in MyProject
    //if(key=='q') ; // used in MyProject
    //if(key=='r') ; // used in MyProject
    ...
    //if(key=='E') ;
    //if(key=='F') ;
    if(key=='G') Sites.placeWxWpointsOnQuadGrid(5); 
    if(key=='H') Sites.placeWxWpointsOnHexGrid(5); 
    if(key=='I') showIDs=!showIDs; 
    //if(key=='J') ;
    if(key=='K') ;
    if(key=='L') ;  // used in MyProject
    if(key=='M') M.left();
    if(key=='N') M.next();
    if(key=='O') M.opposite();
    if(key=='P') M.previous(); 
...
void mousePressed()   // executed when the mouse buttom was just pressed
  ...
  if (keyPressed) 
     {
     if (key=='n')  Sites.addPoint(Mouse()); // appends NEW vertex after the last one
...
void mouseDragged() // executed when the mouse is dragged (while mouse buttom pressed)
  ...
  if (keyPressed) {
      ...
      if (key=='m') Sites.translate(); // move all vertices PNTSfromNxNGrid(P[0]).translate();
      if (key=='p') Sites.translate(0,8); // move all vertices PNTSfromNxNGrid(P[0]).translate();
      if (key=='q') Sites.translate(9,17); // move all vertices PNTSfromNxNGrid(P[0]).translate();
      if (key=='r') Sites.translate(18,26); // move all vertices PNTSfromNxNGrid(P[0]).translate();
